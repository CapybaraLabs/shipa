...
import dev.capybaralabs.shipa.discord.gateway.Gateway.GatewayConnectionState
import dev.capybaralabs.shipa.discord.gateway.Gateway.PreLaunch
import dev.capybaralabs.shipa.discord.gateway.OutboundGatewayCloseCode.*
...
/**
 * Launch Gateways as coroutines & handle their lifecycle / errors.
 */
class GatewayScope(
	...
	private val infrastructure = ConnectionInfrastructure(config, mapper)

	@OptIn(ExperimentalCoroutinesApi::class)
	// TODO configurable / finetuning defaults
	private val scope: CoroutineScope = CoroutineScope(Dispatchers.IO.limitedParallelism(10))

	suspend fun supervise() {

		// TODO this doesnt do anything. how do we relaunch the gateway when it fails eventually?
	...
	private suspend fun launchGateway(): Gateway {
		try {
			val stateHolder = StateHolder(config, PreLaunch(config))
			val actor: SendChannel<InternalGatewayEvent> = scope.gatewayActor { stateHolder.state() }
			val handler = WebsocketHandlerAdapter(mapper, actor, config)

			val gateway = Gateway(config, scope, identifyLimiter, dispatchHandler, stateHolder, infrastructure, handler)

			// TODO race condition? Could something attempt to access the session before it's set?
			stateHolder.newSession(infrastructure.createConnection(handler))

			return gateway
		} catch (e: Exception) {
			// TODO do we eventually have a stackoverflow due to the concurrent call?
			logger().warn("Gateway failed, relaunching", e)
	...
	private fun CoroutineScope.gatewayActor(state: () -> GatewayConnectionState) = actor<InternalGatewayEvent>(
		capacity = 100, // TODO configurable?
	) {
		// single threaded processing in here
		for (event in channel) {
			// TODO what happens if we throw in this coroutine?
			state().handleEvent(event)
		}

		logger().info("{} Gateway actor terminated", config.renderShardInfo())
	}
}


private class ConnectionInfrastructure(
	private val config: StaticGatewayConfig,
	private val mapper: ObjectMapper,
	private val client: WebSocketClient = StandardWebSocketClient(),
) {

	suspend fun createConnection(handler: WebsocketHandlerAdapter): WebSocketSessionSender {
		// TODO consider refreshing the URI from the static config
		return connect(config.gatewayUri, handler)
	}

	suspend fun resumeConnection(resumeUri: URI, handler: WebsocketHandlerAdapter): WebSocketSessionSender {
		return connect(resumeUri, handler)
	}

	suspend fun connect(uri: URI, handler: WebsocketHandlerAdapter): WebSocketSessionSender {
...
private class StateHolder(
	...
	private var state: GatewayConnectionState = initialState
	private lateinit var session: WebSocketSessionSender

	fun state(): GatewayConnectionState {
		return state
	}

	fun session(): WebSocketSessionSender {
		return session
	}

	fun nextState(nextState: GatewayConnectionState) {
		logger().debug("{} Transitioning state from {} to {}", config.renderShardInfo(), state, nextState)
		this.state = nextState
	}

	fun newSession(newSession: WebSocketSessionSender) {
		logger().debug("{} Replacing session", config.renderShardInfo())
		//	this.session.close() TODO try attempt close to avoid resource leaks
...
private class Gateway(
	...
	private val infrastructure: ConnectionInfrastructure,
	private val handler: WebsocketHandlerAdapter,
) {

	init {
		stateHolder.nextState(Launching())
	}

	private fun state(): GatewayConnectionState {
		return stateHolder.state()
	}

	private fun session(): WebSocketSessionSender {
		return stateHolder.session()
	}

	private fun nextState(nextState: GatewayConnectionState) {
		stateHolder.nextState(nextState)
	}

	private fun newSession(nextState: GatewayConnectionState, newSession: WebSocketSessionSender) {
		stateHolder.nextState(nextState)
		stateHolder.newSession(newSession)
	}
	...
	class PreLaunch(private val config: StaticGatewayConfig) : GatewayConnectionState {
	...
	abstract inner class GatewayConnectionBehaviour : GatewayConnectionState {
		...
		override suspend fun handleEvent(event: InternalGatewayEvent) {
			...
			when (event) {
				...
				is ConnectionClosed -> {
					...
					if (closeCode == ReconnectNoResume) {
						resumeOrReconnect(resumeAllowed = false)
					} else {
						resumeOrReconnect()
					}
					return
				...
				// Discord Events
				is InvalidSession -> {
					val closeStatus = if (!event.resumable) {
						logger().warn("{} Invalid session and NOT resumable during {}", config.renderShardInfo(), state())
						ReconnectNoResume
					} else {
						logger().info("{} Invalid session but resumable during {}", config.renderShardInfo(), state())
						ResumeOrReconnect
					}
					session().close(closeStatus, "Invalid Session received from Discord")
					return
				}

				is Reconnect -> {
					logger().info("{} Reconnect requested during {}", config.renderShardInfo(), state())
					session().close(ResumeOrReconnect, "Reconnect requested by Discord")
					return
				}
		...
		// only call from connection closed. otherwise need to think about destroying ongoing connection.
		private suspend fun resumeOrReconnect(resumeAllowed: Boolean = true) {
			...
			if (resumeAllowed
				&& !alreadyResuming
				&& identified != null && sequence != null
			) {
				// Resume
				val nextState = ResumeAwaitingHello(identified.heartbeatState, identified.resumeGatewayUrl, identified.sessionId, sequence)
				val newSession = infrastructure.resumeConnection(identified.resumeGatewayUrl, handler)
				newSession(nextState, newSession)
			} else {
				// Reconnect
				val newSession = infrastructure.createConnection(handler)
				newSession(Reconnecting(), newSession)
			}
		}
	}

	inner class Launching : GatewayConnectionBehaviour() {
		...
		override suspend fun handleEvent(event: InternalGatewayEvent) {
			...
			when (event) {
				ConnectionEstablished -> {
					logger().info("{} Connected gateway", config().renderShardInfo())
					nextState(ConnectAwaitingHello())
				}
		...
		protected fun CoroutineScope.launchHeartbeatLoop(interval: Duration, sequenceRef: AtomicReference<Int?>, ackRef: AtomicBoolean): Job = launch {
			...
			while (isActive) {
				...
				if (ThreadLocalRandom.current().nextBoolean()) {
					throw java.lang.IllegalStateException("Random exception for testing purposes")
				}
			}
		}
	}
		...
		override suspend fun handleEvent(event: InternalGatewayEvent) {
			when (event) {
				is Hello -> {
					...
					nextState(AwaitingReady(heartbeatState))
	...
	inner class AwaitingReady(
		...
		override suspend fun handleEvent(event: InternalGatewayEvent) {
			when (event) {
				is Dispatch.Ready -> {
					updateSequence(event.sequence)
					nextState(Connected(heartbeatState, URI.create(event.data.resumeGatewayUrl), event.data.sessionId))
				}
		...
		override suspend fun handleEvent(event: InternalGatewayEvent) {
			when (event) {
				...
				is Hello -> {
					...
					nextState(Resuming(freshHeartbeat, resumeGatewayUrl, sessionId))
		...
		override suspend fun handleEvent(event: InternalGatewayEvent) {
			when (event) {
				...
				is Resumed -> {
					...
					nextState(Connected(heartbeatState, resumeGatewayUrl, sessionId))
					return
				}
...
//		// Used for testing / debugging reconnect behaviour.
//		GlobalScope.async {
//			delay(20_000)
//			tryTell(InvalidSession(false))
//		}
	...
	override fun afterConnectionClosed(session: WebSocketSession, status: CloseStatus) {
		val closeCode = InboundGatewayCloseCode.values().find { it.value == status.code }
			?: values().find { it.value == status.code }
